...
export const CategoryProvider = ({ children } : {children: React.ReactNode}) => {
    ...
    const [originalCategories, setOriginalCategories] = useState<categoryWithImageAndUrlType[] | undefined>(finalCategories);
    const [filteredCategories, setFilteredCategories] = useState<categoryWithImageAndUrlType[] | undefined>(finalCategories);
    const [searchTerm, setSearchTerm] = useState('');
    const [sortFilter, setSortFilter] = useState('');
    const [audScoreFilter, setAudScoreFilter] = useState({fresh: false, rotten: false});
    ...
    const applyFiltersAndSorts = (categories: categoryWithImageAndUrlType[] | undefined) => {
      ...
      if (sortFilter === 'AToZ') {
        ...
        result = result.slice().sort((a, b) => (b.criticScore ?? 0) - (a.criticScore ?? 0));
      } else if (sortFilter === 'lowest') {
        result = result.slice().sort((a, b) => (a.criticScore ?? 0) - (b.criticScore ?? 0));
      } else if (sortFilter === 'audHighest') {
        result = result.slice().sort((a, b) => (b.audienceScore ?? 0) - (a.audienceScore ?? 0));
      } else if (sortFilter === 'audLowest') {
        result = result.slice().sort((a, b) => (a.audienceScore ?? 0) - (b.audienceScore ?? 0));
      }
      ...
      if (audScoreFilter.fresh && audScoreFilter.rotten) {;}
        ...
        result = result.filter(category => category.audienceScore !== null && category.audienceScore !== undefined && category.audienceScore < 60);
      }
    ...
    const toggleAudScore = (filter: string) => {
      const newObj = {...audScoreFilter, [filter]: !audScoreFilter[filter as 'fresh' | 'rotten']};
  ...
  return (
    <CategoryContext.Provider value={{ filteredCategories, filterCategories, sortCategories, toggleAudScore }}>
      {children}
